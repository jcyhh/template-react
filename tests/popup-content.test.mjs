...
test('popup content exports preset content wrappers by popup position', async () => {
    ...
    assert.match(styles, /\.popup-content\s*\{[\s\S]*isolation:\s*isolate;/)
    assert.match(styles, /&::before\s*\{[\s\S]*z-index:\s*0;/)
    assert.match(styles, /> \*\s*\{[\s\S]*position:\s*relative;[\s\S]*z-index:\s*1;/)
